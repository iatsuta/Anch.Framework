...
public class InjectMaybeVisitor : ExpressionVisitor
{
    ...
    protected override Expression VisitListInit(ListInitExpression node)
    {
        return new[] { node.NewExpression }.Concat(node.Initializers.SelectMany(i => i.Arguments)).OverrideSelect(innerSource =>
        {
            ...
            var initializers = node.Initializers.Select(i => i.Update(enumerator.ReadMany(i.Arguments.Count))).ToArray();
    ...
    {
        return new[] { node.NewExpression }.Concat(node.Bindings.SelectMany(ex => ex.GetMemberBindingExpressions())).OverrideSelect(innerSource =>
        {
            using var enumerator = innerSource.Select(v => v).GetEnumerator();
            var newExpr = (NewExpression)enumerator.ReadSingle();

            var initializers = node.Bindings.Select(i => i.UpdateMemberBindingBase(enumerator)).ToArray();
