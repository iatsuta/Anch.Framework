...
public class AppDbContext(DbContextOptions<AppDbContext> options, IMainConnectionStringSource mainConnectionStringSource) : DbContext(options)
{
    ...
    private void InitApp(ModelBuilder modelBuilder)
    {
        ...
        {
            var entity = modelBuilder.Entity<ManagementUnit>().ToTable(nameof(ManagementUnit), DefaultSchema);
            entity.HasKey(v => v.Id);

            entity.Property(e => e.Name).IsRequired().HasMaxLength(DefaultMaxLength);
            entity.HasOne(e => e.Parent).WithMany().HasForeignKey($"{nameof(ManagementUnit.Parent)}{DefaultIdPostfix}").IsRequired(false);
        }
    }
    ...
    private void InitAncestors(ModelBuilder modelBuilder)
	{
        ...
        {
            var entity = modelBuilder.Entity<ManagementUnitDirectAncestorLink>().ToTable(nameof(ManagementUnitDirectAncestorLink), DefaultSchema);
            entity.HasKey(v => v.Id);

            var ancestorKey = $"{nameof(ManagementUnitDirectAncestorLink.Ancestor)}{DefaultIdPostfix}";
            var childKey = $"{nameof(ManagementUnitDirectAncestorLink.Child)}{DefaultIdPostfix}";
        ...
        {
            var entity = modelBuilder.Entity<ManagementUnitUndirectAncestorLink>().ToView(nameof(ManagementUnitUndirectAncestorLink), DefaultSchema);
            entity.HasNoKey();

            entity.HasOne(e => e.Source).WithMany().HasForeignKey($"{nameof(ManagementUnitUndirectAncestorLink.Source)}{DefaultIdPostfix}").IsRequired();
            entity.HasOne(e => e.Target).WithMany().HasForeignKey($"{nameof(ManagementUnitUndirectAncestorLink.Target)}{DefaultIdPostfix}").IsRequired();
        }
    }
