...
{
    public async Task<AccessorsFilterInfo<TDomainObject>> CreateFilterAsync(DomainSecurityRule.RoleBaseSecurityRule securityRule, SecurityPath<TDomainObject> securityPath, CancellationToken ct)
    {
        ...
        CancellationToken ct)
    {
        ...
        var getAccessorsFunc = LazyHelper.Create(() => FuncHelper.Create((TDomainObject domainObject) =>
        {
            ...
            return permissionSystem
                .GetPermissionSources(securityRule)
                .ToAsyncEnumerable()
                .SelectMany(permissionSource => permissionSource.GetAccessorsAsync(filter))
                .Distinct();
        }));
